...
public:
    ...
    int dp(int i, long long mask, bool canChange) {
        ...
        if (__builtin_popcountll(newMask) > k) res = 1 + dp(i+1, 1LL << bit, canChange);
        else res = dp(i+1, newMask, canChange);

        if (canChange) {
            for (int j = 0; j<26; ++j) {
                long long changeMask = mask | (1LL << j);
                if (__builtin_popcountll(changeMask) > k) res = max(res, 1 + dp(i+1, 1LL << j, false));
                else res = max(res, dp(i+1, changeMask, false));
            }
        }
